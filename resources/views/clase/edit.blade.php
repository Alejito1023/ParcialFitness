...
<html lang="en">
  ...
  <body>
    <div class="container">
    <h1>Add Clase</h1>
    <form method="POST" action="{{ route('clases.store',['clase'=>$clase->clase_id])}}">
        @method('put')
        @csrf
        <div class="mb-3">
          ...
          <label for="entrenadores">Suscripcion:</label>
        <select class="form-select" id="entrenadores" name="code" required>
              <option selected disabled value="">Choose one...</option>
              @foreach ($entrenadores as $entrenador)
              @if ($entrenador->entrenador_id == $clase->clase_id) 
              <option selected value="{{ $entrenador->entrenador_id}}">{{ $entrenador->entrenador_nom}}</option>
              @else
